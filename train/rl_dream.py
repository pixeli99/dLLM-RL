...
def main():
    ...
    def forward_process(
        ...
        logp_new_tok = F.log_softmax(shift_logits, dim=-1) \
                  .gather(-1, shift_labels.unsqueeze(-1)).squeeze(-1)  # (B, T-1)

        ratio   = torch.exp(logp_new_tok - logp_old_tok)          # (B, T-1)
        ...
        if config.training.beta > 0:
            ...
            kl_seq = (kl_seq * shift_mask).sum(dim=1)
